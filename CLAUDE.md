...
**CI / gates** (`.github/workflows/`). This is a summary — **`docs/dev-loop-mechanisms.md` is the authoritative, per-step ledger** (what drives each stage and what enforces it), kept current as the loop changes; consult it before touching any workflow, and update it in the same change. The nine workflows:
