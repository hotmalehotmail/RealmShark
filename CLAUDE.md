...
**CI / gates** (`.github/workflows/`). This is a summary — **`docs/dev-loop-mechanisms.md` is the authoritative, per-step ledger** (what drives each stage and what enforces it), kept current as the loop changes; consult it before touching any workflow, and update it in the same change. The eleven workflows:
...
- `sweep.yml` — a level-triggered safety net (cron ~30 min) that merges any green, idle, un-armed `claude/*` PR the edge-triggered gatekeeper stranded, under the same restrictions (same-repo, no `--admin`, soak-freeze respected); a conflicted (`DIRTY`) candidate is re-fired through the same rebase script as conflict-watch's backstop instead of skipped.
