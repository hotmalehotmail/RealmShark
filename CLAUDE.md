...
Overlay tests: `npm test` (vitest, `overlay/test/`) — capture-replay regression tests (real/synthetic soak captures under `overlay/test/fixtures/captures/`, replayed through `overlay/test/replay.ts` with fake timers anchored to each envelope's `time`) plus an allowlist tripwire. See `docs/overlay-testing.md`. Every repro capture from a bug report or failed soak becomes a committed fixture + a failing-then-green test in the same PR as the fix.
...
**CI / gates** (`.github/workflows/`). This is a summary — **`docs/dev-loop-mechanisms.md` is the authoritative, per-step ledger** (what drives each stage and what enforces it), kept current as the loop changes; consult it before touching any workflow, and update it in the same change. The twelve workflows:
- `ci.yml` — the required merge gate. Overlay `typecheck`+`lint`+`npm test` (vitest; Node 22, `npm ci --ignore-scripts` — skips the native rebuild the Linux runner can't do) and `gradle test` (JUnit DPS tests) + `gradle bridgeJar`. Gradle 7.4.2 is pinned via `gradle/actions/setup-gradle` (no committed wrapper — `gradle/` is gitignored and Shadow 7.0.0 breaks on Gradle 9). A third **`workflow parity`** job fails any PR whose `review.yml` differs from the default branch — guarding the silent review-skip (a stale `review.yml` makes the reviewer skip green).
