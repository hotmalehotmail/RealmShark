...
**Branch model.** `feature/*` / `fix/*` → `staging` (integration; alpha prereleases) → `bridge` (stable trunk; beta releases). **`bridge` is the GitHub default branch and the base for PRs** — not the stale `origin/main` or the upstream `realmshark` mirror. `staging` and `bridge` are both protected. Required checks on both: the two `ci` jobs (`overlay — typecheck + lint`, `bridge — compile + fat jar`), `review-verdict`, and `workflow parity`; 0 human approvals; `enforce_admins` is off (the deliberate owner escape hatch — you can `--admin` merge a blocked PR).
...
**CI / gates** (`.github/workflows/`). This is a summary — **`docs/dev-loop-mechanisms.md` is the authoritative, per-step ledger** (what drives each stage and what enforces it), kept current as the loop changes; consult it before touching any workflow, and update it in the same change. The ten workflows:
...
- `review.yml` — an independent code-review agent (Opus 4.8) on each PR, scoring **correctness + code quality + docs freshness**. It posts inline findings and a machine-readable **`review-verdict`** commit status (`success`/`failure`) — the gate the rest of the loop reads. Authed via `CLAUDE_CODE_OAUTH_TOKEN` + the Claude GitHub App; needs `id-token: write`. Runs only when `review.yml` is byte-identical to the default branch (so it cannot review the PR that changes it; also skips a `staging → bridge` promotion). A skip posts **no** verdict, so the required `review-verdict` check fails closed rather than reading green.
- `gatekeeper.yml` — **live** (no longer an `if: false` stub). On `ci`/`review` completion it arms GitHub native auto-merge for a green `claude/*` PR into `staging` (using `MERGE_PAT` so the merge fires downstream events), **holds** unrelated agent PRs while an alpha soak is open, and re-fires the build agent in FIX MODE to rebase on a merge conflict.
- `sweep.yml` — a level-triggered safety net (cron ~30 min) that merges any green, idle, un-armed `claude/*` PR the edge-triggered gatekeeper stranded, under the same restrictions (same-repo, no `--admin`, soak-freeze respected).
- `fixloop.yml` / `resume.yml` — the review-fix loop. On `review-verdict = failure` the build agent is re-fired in FIX MODE, capped at 3 attempts (marker-comment budget) before it escalates to `agent:needs-human`; applying `agent:retry` resets the budget and re-fires.
...
- `soak-verdict.yml` / `post-merge.yml` — the alpha-soak release loop. `soak:pass` promotes `staging → bridge` (a PR with a stamped `review-verdict`, via `MERGE_PAT`); `soak:fail` fixes forward; a merge during a soak auto-cuts a fresh alpha, and the landed promotion cuts the latest release.
